...
QSqlQueryModel * Client::liste_reclamation(QString val)
{
    QSqlQueryModel* model=new  QSqlQueryModel();
          ...
          model->setQuery("SELECT client.cin ,reclamation.date_r,client.nom ,client.prenom ,reclamation.etat ,reclamation.sujet FROM client FULL JOIN reclamation ON client.cin=reclamation.id_r WHERE cin LIKE'"+val+"%'");
...
QSqlQueryModel * Client::liste_clients(QString d1, QString d2)
{
    ...
    //QDate date2=QDate::fromString(d2,"MM/dd/yyyy");
          ...
          model->setQuery("SELECT client.cin ,gs_rdv.date_rdv ,client.nom, client.prenom ,gs_rdv.heure_rdv ,gs_rdv.region  FROM client FULL JOIN gs_rdv ON client.cin=gs_rdv.id_c WHERE date_rdv BETWEEN  '"+d1+"' AND '"+d2+"'");
return model;
...
{   int sum=0;
    QSqlQuery query;
    query.prepare("SELECT  FROM client where sexe like 'femme'");
...
{   int sum=0;
    QSqlQuery query;
    query.prepare("SELECT * FROM client sexe like 'homme' ");
