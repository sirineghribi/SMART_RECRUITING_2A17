...
bool Rdv::ajouter()
{
   ...
   /* query.prepare("INSERT INTO GS_RDV (ID_RDV, DATE_RDV, HEURE_RDV, REGION, ID_P, ID_C) "
                            "VALUES (:ID_RDV, :DATE_RDV, :HEURE_RDV, :REGION, :ID_P, :ID_C)"
                  "INNER JOIN partenaire ON GS_RDV.ID_P = partenaire.ID INNER JOIN client ON GS_RDV.ID_C = client.CIN ");*/

    //query.prepare("INSERT INTO GS_RDV ( ID_RDV, DATE_RDV, HEURE_RDV, REGION, ID_P, ID_C) SELECT * FROM GS_RDV p INNER JOIN client c ON c.CIN = p.ID_C INNER JOIN partenaire n ON p.ID_P = n.ID");
...
QSqlQueryModel* Rdv::afficher()
{
    QSqlQueryModel* model=new QSqlQueryModel();
          ...
          model->setQuery("SELECT* FROM GS_RDV INNER JOIN PARTENAIRE ON GS_RDV.ID_P = PARTENAIRE.ID INNER JOIN CLIENT ON GS_RDV.ID_C = CLIENT.CIN");
...
QSqlQueryModel* Rdv::afficher_partenaire()
{
    QSqlQueryModel* model=new QSqlQueryModel();
          ...
          model->setQuery("SELECT* FROM GS_RDV INNER JOIN partenaire ON GS_RDV.ID_P = partenaire.ID");
          model->setHeaderData(0, Qt::Horizontal, QObject::tr("ID_RDV"));
          model->setHeaderData(1, Qt::Horizontal, QObject::tr("DATE_RDV"));
          model->setHeaderData(2, Qt::Horizontal, QObject::tr("HEURE_RDV"));
          model->setHeaderData(3, Qt::Horizontal, QObject::tr("REGION"));
